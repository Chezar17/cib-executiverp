...
// ══════════════════════════════════════════════════════════════
//  WANTED LIST API  —  /api/wanted
//
//  GET    /api/wanted           → all active wanted entries (public)
//  POST   /api/wanted           → create entry  (secret+ only)
//  PUT    /api/wanted           → update entry  (secret+ only)
//  DELETE /api/wanted?id=xxx    → soft-delete   (secret+ only)
//
//  ── Supabase table DDL (run once) ──────────────────────────
//
//  CREATE TABLE wanted_individuals (
//    id             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
//    case_no        TEXT NOT NULL,           -- e.g. CIB-MW-2026-003
//    suspect_id     TEXT,                    -- e.g. SS-02
//    wanted_since   DATE NOT NULL,
//    full_name      TEXT NOT NULL,
//    alias          TEXT,
//    dob            TEXT,
//    nationality    TEXT,
//    bounty         TEXT NOT NULL,           -- e.g. $10,000
//    bounty_note    TEXT,
//    debrief        TEXT NOT NULL,
//    last_location  TEXT,
//    affiliation    TEXT,
//    threat_level   TEXT NOT NULL DEFAULT 'critical',  -- critical|high|medium|low
//    status         TEXT NOT NULL DEFAULT 'At Large',  -- At Large|Captured|Deceased
//    crimes         TEXT NOT NULL DEFAULT '[]',        -- JSON array of strings
//    det_name       TEXT,
//    det_rank       TEXT,
//    det_division   TEXT,
//    photo_url      TEXT,
...
const WANTED_TABLE  = 'wanted_individuals'
const WANTED_BUCKET = 'wanted-photos'

async function uploadWantedPhoto(supabase, base64Data, existingUrl) {
  if (!base64Data) return existingUrl || null
  try {
    const matches = base64Data.match(/^data:(.+);base64,(.+)$/)
    if (!matches) return existingUrl || null
    ...
    const { data, error } = await supabase.storage
      .from(WANTED_BUCKET)
      .upload(fileName, buffer, { contentType: mimeType, upsert: false })
    if (error) { console.error('Wanted photo upload error:', error); return existingUrl || null }
    const { data: urlData } = supabase.storage.from(WANTED_BUCKET).getPublicUrl(data.path)
    return urlData.publicUrl
  } catch (err) {
    console.error('Wanted photo error:', err)
    return existingUrl || null
  }
}

export async function wantedHandler(req, res) {
  ...
  try {
    ...
    // POST — create
    if (req.method === 'POST') {
      const { case_no, suspect_id, wanted_since, full_name, alias, dob, nationality,
              bounty, bounty_note, debrief, last_location, affiliation, threat_level,
              status, crimes, det_name, det_rank, det_division,
              photo_base64, photo_url: photo_url_direct } = req.body
      ...
      const photo_url = photo_base64
        ? await uploadWantedPhoto(supabase, photo_base64, null)
        : (photo_url_direct || null)

      const { data, error } = await supabase
        .from(WANTED_TABLE)
        .insert([{
          case_no, suspect_id, wanted_since, full_name, alias, dob, nationality,
          bounty, bounty_note, debrief, last_location, affiliation,
          threat_level: threat_level || 'critical',
          status: status || 'At Large',
          crimes: crimes || '[]',
          det_name, det_rank, det_division, photo_url,
          created_by: actor
        }])
        .select().single()
    ...
    // PUT — update
    if (req.method === 'PUT') {
      const { id, photo_base64, photo_url: photo_url_direct, ...rest } = req.body
      if (!id) return res.status(400).json({ error: 'ID is required for update' })

      const { data: oldData } = await supabase.from(WANTED_TABLE).select('photo_url').eq('id', id).single()
      const photo_url = photo_base64
        ? await uploadWantedPhoto(supabase, photo_base64, oldData?.photo_url)
        : (photo_url_direct || oldData?.photo_url || null)

      const { data, error } = await supabase
        .from(WANTED_TABLE)
        .update({ ...rest, photo_url })
        .eq('id', id)
        .select().single()
    ...
    if (req.method === 'DELETE') {
      ...
      const { error } = await supabase.from(WANTED_TABLE).update({
        is_deleted: true, deleted_by: actor,
        deleted_at: new Date().toISOString()
      }).eq('id', id)
